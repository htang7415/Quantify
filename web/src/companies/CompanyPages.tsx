    ...
    <main className="data-app company-detail-page">
      ...
      <div className="detail-shell">
        ...
        {etfExposures.length > 0 && <section className="terminal-module company-etf-module" aria-labelledby="company-etf-title">
          ...
          <div className="holdings-scroll"><table className="holdings-table company-etf-table"><thead><tr><th>Fund</th><th>Filed position value</th><th>Filed weight</th><th>Shares</th><th>Report date</th><th>Source</th></tr></thead><tbody>{etfExposures.map(({ fund, holding }) => <tr key={`${fund.fund_id}-${holding.holding_id}`}><td><a href={`/markets/etfs/${fund.slug}`}><strong>{fund.ticker}</strong><span>{fund.name}</span></a></td><td>{money(holding.currency_value)}</td><td><strong>{holding.filed_percentage.toFixed(2)}%</strong></td><td>{Math.round(holding.balance).toLocaleString("en-US")}</td><td>{fund.report_date}</td><td><a className="table-source-link" href={fund.source_url} target="_blank" rel="noreferrer">N-PORT ↗</a></td></tr>)}</tbody></table></div>
