...
export async function verifyAnalysis(
  ...
  if (!authenticatedEndpoint && !trialEndpoint) {
    ...
    throw new Error("Quantify verification is not configured for this environment.");
  }
...
export function parseSafeResponse(payload: unknown): VerificationResponse {
  if (!payload || typeof payload !== "object") throw new Error("Quantify returned an invalid verification result.");
  const value = payload as Record<string, unknown>;
  if (Object.keys(value).length !== responseKeys.size || !Object.keys(value).every((key) => responseKeys.has(key))) {
    throw new Error("Quantify returned an unsafe verification result.");
  }
  ...
  if (!Array.isArray(rawVerdicts) || !evidence || typeof evidence !== "object") {
    throw new Error("Quantify returned an invalid verification result.");
  }
  const safeVerdicts = rawVerdicts.map((item) => {
    if (!item || typeof item !== "object") throw new Error("Quantify returned an invalid verification result.");
