...
const catalogs = new Set<PublicCatalog>(["investors", "markets", "macro", "rates", "etf_flows", "crypto", "crypto_exposure", "earnings", "policy", "events"]);
...
export function parsePublicReleaseIndex(value: unknown): PublicReleaseIndex {
  ...
  if (index.schema_version !== "public-release-index.v1") throw new Error("Public release index schema is unsupported.");
