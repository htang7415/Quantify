      ...
      <section className="earnings-company-grid page-shell" aria-labelledby="earnings-release-title">
        ...
        <div className="earnings-card-grid">
          {earningsCatalog.companies.map((company) => <article className="earnings-card" key={company.ticker}>
            ...
            <div className="earnings-metrics"><div><span>Revenue</span><strong>{money(company.revenue.value)}</strong><b>↑ {company.revenue.yoy_change_pct.toFixed(1)}% YoY</b></div><div><span>Diluted EPS</span><strong>${company.diluted_eps.value.toFixed(2)}</strong><b>↑ {company.diluted_eps.yoy_change_pct.toFixed(1)}% YoY</b></div></div>
