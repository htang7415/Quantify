...
from quantify.indexed_release import IndexedEvidenceRelease, IndexedSnapshotRequest, IndexedReleaseError
...
class IndexedReleaseArchive:
    ...
    def dump(release: IndexedEvidenceRelease) -> bytes:
        ...
        payload={"schema_version":_VERSION,"evidence_release":release.evidence_release.as_dict(),"indexed_release_manifest_hash":release.manifest_hash,"snapshots":snapshots}
    ...
    def load(payload: bytes) -> "ArchivedIndexedSnapshotProvider":
        ...
        if not isinstance(raw,dict) or raw.get("schema_version")!=_VERSION or not isinstance(raw.get("snapshots"),list): raise IndexedReleaseError("indexed release archive schema is invalid")
        ...
        declared=release_raw.pop("manifest_hash",None)
        try: release=EvidenceRelease(**release_raw)
        ...
        return ArchivedIndexedSnapshotProvider(raw=raw)
...
class ArchivedIndexedSnapshotProvider:
    def __init__(self, *, raw: Mapping[str, object]) -> None:
        ...
        for item in raw["snapshots"]:
            request_raw=item["request"]; request=IndexedSnapshotRequest(cik=request_raw["cik"],as_of_date=date.fromisoformat(request_raw["as_of_date"]),forms=tuple(request_raw["forms"]),acquisition_records=tuple(tuple(row) for row in request_raw["acquisition_records"]))
            ...
            sel=item["selection"]; selection=RestatementSelection(policy=RestatementPolicy(sel["policy"]),as_of_date=date.fromisoformat(sel["as_of_date"]),selected_evidence_ids=tuple(sel["selected"]),superseded_evidence_ids=tuple(sel["superseded"]),eligibility_decisions=tuple(EvidenceEligibilityDecision(evidence_id=x,reason=EvidenceEligibilityReason(y)) for x,y in sel["decisions"]))
            ...
            self._by_request[request.key()]=SnapshotBuild(snapshot=snapshot,selection=selection,audit_manifest=audit)

    def build(self, *, cik: str, as_of_date: date, forms: tuple[str,...], acquisition_records=()) -> SnapshotBuild:
        request=IndexedSnapshotRequest(cik=cik,as_of_date=as_of_date,forms=forms,acquisition_records=tuple((r.request_type.value,r.reason) for r in acquisition_records))
        try: return self._by_request[request.key()]
        except KeyError as error: raise IndexedReleaseError("request is not compiled into the declared release") from error
